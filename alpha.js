...
const { state, saveCreds } = await useMultiFileAuthState('./Session')
    ...
    alpha.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./store")(alpha, m, chatUpdate, store, opengc, antilink, antiwame, antilink2, antiwame2, set_welcome_db, set_left_db, set_proses, set_done, set_open, set_close, _welcome, _left, db_respon_list)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    alpha.ev.on('group-participants.update', async (anu) => {
    	...
    	alpha.sendMessage(anu.id, { mentions: [num], text: `Selamat Datang @${num.split("@")[0]} di Group ${metadata.subject}` })
                }
                  ...
                  if (isSetLeft(anu.id, set_left_db)) {            	
                      ...
                      alpha.sendMessage(anu.id, {text: `${full_pesan}`})
                       	...
                       	alpha.sendMessage(anu.id, { mentions: [num], text: `Bye @${num.split("@")[0]}` })
                        }
                        } else if (anu.action == 'promote') {
                    alpha.sendMessage(anu.id, { mentions: [num], text: `@${num.split('@')[0]} sekarang menjadi admin grup ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    alpha.sendMessage(anu.id, { mentions: [num], text: `@${num.split('@')[0]} bukan admin grup ${metadata.subject} lagi` })
              }
            }
        } catch (err) {
            console.log(err)
